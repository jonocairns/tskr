...
export function PointsSummary({
  ...
  const progress = Math.min(100, Math.max(0, Math.round((points / threshold) * 100)));
  ...
  return (
    <Card className="border-primary/10 bg-gradient-to-br from-primary/5 via-background to-background shadow-sm">
      <CardHeader className="flex flex-row items-center justify-between space-y-0">
        <div>
          <CardTitle className="text-xl font-semibold">Points overview</CardTitle>
          <CardDescription>Log tasks, climb the leaderboard, claim rewards.</CardDescription>
        </div>
      ...
      <CardContent className="space-y-4">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="text-sm text-muted-foreground">
            Threshold: <span className="font-medium text-foreground">{threshold} pts</span>
          </div>
        </div>
...
function Stat({
  ...
  return (
    <div className="space-y-1">
      ...
      <p className={`text-xl font-semibold ${muted ? "text-muted-foreground" : ""}`}>
        {value}
      </p>
    </div>
  );
