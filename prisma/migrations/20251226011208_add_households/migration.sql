/*
  ...
  - Added the required column `householdId` to the `PointLog` table without a default value. This is not possible if the table is not empty.
  - Added the required column `householdId` to the `PresetTask` table without a default value. This is not possible if the table is not empty.
...
CREATE TABLE "_UserHouseholdMap" (
    "userId" TEXT NOT NULL PRIMARY KEY,
    "householdId" TEXT NOT NULL
);

INSERT INTO "_UserHouseholdMap" ("userId", "householdId")
...
SELECT
    map."householdId",
    COALESCE("User"."name", "User"."email", 'Household'),
    map."userId",
    CURRENT_TIMESTAMP,
    "User"."updatedAt"
FROM "_UserHouseholdMap" map
JOIN "User" ON "User"."id" = map."userId";
...
INSERT INTO "new_PointLog" ("createdAt", "description", "duration", "durationMinutes", "id", "kind", "points", "presetId", "presetKey", "revertedAt", "revertedById", "rewardCost", "updatedAt", "userId", "householdId")
SELECT
    "PointLog"."createdAt",
    ...
    map."householdId"
FROM "PointLog"
JOIN "_UserHouseholdMap" map ON map."userId" = "PointLog"."userId";
DROP TABLE "PointLog";
...
INSERT INTO "new_PresetTask" ("bucket", "createdAt", "createdById", "id", "isShared", "label", "updatedAt", "householdId")
SELECT
    "PresetTask"."bucket",
    ...
    map."householdId"
FROM "PresetTask"
JOIN "_UserHouseholdMap" map ON map."userId" = "PresetTask"."createdById";
...
SELECT
    ...
    map."householdId"
FROM "User"
JOIN "_UserHouseholdMap" map ON map."userId" = "User"."id";
DROP TABLE "User";
