...
import { getCurrentStreak } from "@/lib/dashboard/queries";
import { DEFAULT_TIME_ZONE, resolveTimeZone } from "@/lib/timeZones";
...
test("falls back to a safe time zone when input is empty or invalid", () => {
	const resolvedEmpty = resolveTimeZone(" ");
	const resolvedInvalid = resolveTimeZone("Not/A-Real_Zone");
	const validFallbacks = new Set([DEFAULT_TIME_ZONE, "UTC"]);

	expect(validFallbacks.has(resolvedEmpty)).toBe(true);
	expect(validFallbacks.has(resolvedInvalid)).toBe(true);
});

test("returns 0 when no logs exist", () => {
	withNow(at(2024, 1, 10, 12, 0), () => {
		expect(getCurrentStreak([], TIME_ZONE)).toBe(0);
	});
});
...
test("treats multiple logs on the same day as a single streak day", () => {
	withNow(at(2024, 1, 10, 12, 0), () => {
		...
		expect(getCurrentStreak(logs, TIME_ZONE)).toBe(1);
	});
});
...
test("stops streak at the first missing day", () => {
	withNow(at(2024, 1, 10, 12, 0), () => {
		...
		expect(getCurrentStreak(logs, TIME_ZONE)).toBe(2);
	});
});
...
test("does not count yesterday's late-night logs as today", () => {
	withNow(at(2024, 1, 10, 0, 30), () => {
		...
		expect(getCurrentStreak(logs, TIME_ZONE)).toBe(0);
	});
});
