...
test("keeps explicit ordering when some visible presets are missing order rows", () => {
	const presets = [
		makePreset("shared-older", "2024-01-01T00:00:00.000Z"),
		makePreset("mine-a", "2024-01-02T00:00:00.000Z"),
		makePreset("mine-b", "2024-01-03T00:00:00.000Z"),
	];

	const ordered = applyUserPresetOrdering(presets, [
		{ presetId: "mine-b", sortOrder: 0 },
		{ presetId: "mine-a", sortOrder: 1 },
	]);

	expect(ordered.map((preset) => preset.id)).toEqual(["mine-b", "mine-a", "shared-older"]);
	expect(ordered.map((preset) => preset.sortOrder)).toEqual([0, 1, 2]);
});

test("preserves explicit precedence even when stored sort orders are sparse", () => {
	const presets = [
		makePreset("preset-a", "2024-01-01T00:00:00.000Z"),
		makePreset("preset-b", "2024-01-02T00:00:00.000Z"),
		makePreset("preset-c", "2024-01-03T00:00:00.000Z"),
	...
	const ordered = applyUserPresetOrdering(presets, [
		{ presetId: "preset-c", sortOrder: 41 },
		{ presetId: "preset-a", sortOrder: 99 },
	]);
